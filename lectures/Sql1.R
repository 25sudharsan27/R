...
View(d1)
sqldf("select * from d1 limit 10")# display 10 rows
sqldf("select Quantity,Profit as 'P' from d1 limit 10")
#filtering the data
sqldf("select Quantity,Discount from d1 where region=='Central' limit 20")
#Sorting
sqldf("select * from d1 order by City desc limit 20") 
sqldf("select City,State,Profit from d1 order by City,State desc limit 20")
...
sqldf("select city,profit from d1 where City like 'He%' ")
sqldf("select city,profit from d1 where City like '%H' ")
sqldf("select city,profit from d1 where City like '%H%' ")
...
#Nested select
sqldf("select region from d1 where  Profit== (Select max(Profit) from Q2)")
...
sqldf("Select sum(Sales) from d1 where Region in ('Central','West') group by Region")
